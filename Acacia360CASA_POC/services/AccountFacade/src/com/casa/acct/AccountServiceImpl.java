...
public class AccountServiceImpl implements AccountService {
	...
	public UserInfoForm checkMemberNo(String memberno) {
		...
		try {
			...
			String name = (String) dbService.execStoredProc(
					"SELECT main.fullname AS name FROM CIFSPBBDB.dbo.TBCIFMAIN main WHERE cifno=:memberno", param,
					null, 0, null);
	...
	public String freezeAccount(Tbfreezeaccount data) {
		...
		try {
			if (check) {
				URL url = new URL(wsurl + "/csr/freeze-account");
				HttpURLConnection con = (HttpURLConnection) url.openConnection();
				con.setDoOutput(true);
				con.setRequestMethod("POST");
				con.setRequestProperty("Content-Type", "application/json");
	...
	public Tbfreezeaccount getFreezeInfo(String accountno) {
		...
		try {
			if (check) {
				String url = wsurl + "/csr/freeze-info/" + accountno;
				...
				if (con.getResponseCode() == HttpURLConnection.HTTP_ACCEPTED) {
					BufferedReader in = new BufferedReader(new InputStreamReader(con.getInputStream()));
					ObjectMapper mapper = new ObjectMapper();
					result = mapper.readValue(in.readLine(), Tbfreezeaccount.class);
				}
				con.disconnect();
			} else {
...
//		List<UserInfoForm> form1 = new ArrayList<UserInfoForm>();
		...
		try {
			...
			form = (CheckDeceasedForm) dbService.execStoredProc(
					"SELECT deceasedflag FROM CIFSPBBDB.dbo.TBCIFMAIN main WHERE cifno=:memberno",
					param, CheckDeceasedForm.class, 0, null);
	...
	public List<Tbchecksforclearing> getFloatItems(String acctno) {
		...
		try {
			...
			list = (List<Tbchecksforclearing>) dbService.execStoredProc(
					"SELECT * FROM Tbchecksforclearing WHERE accountno=:acctno", param, Tbchecksforclearing.class, 1,
					null);
