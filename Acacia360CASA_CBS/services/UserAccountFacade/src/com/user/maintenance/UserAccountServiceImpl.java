...
import com.etel.util.forms.GenericForm;
...
public class UserAccountServiceImpl implements UserAccountService {
	...
	public List<Tbcodetable> mainMod() {
		...
		try { 
			list = (List<Tbcodetable>) dbService.execStoredProc("SELECT id AS id, codevalue AS codevalue, desc1 AS desc1 "
					+ "FROM TBCODETABLE WHERE codename='MODULE'", 
					null, Tbcodetable.class, 1, null);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public List<UserListForm> userList(String username, String instcode) {
		...
		try {
			...
			strquery.append("SELECT usr.id AS id, usr.username AS username, usr.userid AS userid, "
					+ "usr.role AS role, CONCAT(usr.firstname,' ',SUBSTRING(usr.middlename,1,1),'. ',usr.lastname) AS name, "
					+ "unit.brname AS branch "
					+ "FROM TBUSER usr LEFT JOIN TBUNIT unit ON usr.unitbrid=unit.brid ");
			if (username == null) {
			} else {
				strquery.append("AND usr.username LIKE :username");
			}
	...
	@Override
	public UserInfoForm userMainInfo() {
		// TODO Auto-generated method stub
		UserInfoForm form = new UserInfoForm();
		try { 
			param.put("userid", service.getUserId());
			form = (UserInfoForm) dbService.execStoredProc("SELECT usr.firstname AS name, "
					+ "unt.brname AS branch, usr.role AS role, unt.currbusinessdate AS businessdt, "
					+ "usr.unitbrid AS brid FROM TBUSER usr LEFT JOIN TBUNIT unt ON usr.unitbrid=unt.brid "
					+ "WHERE usr.userid=:userid", param, UserInfoForm.class, 0, null);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return form;
