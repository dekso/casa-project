    ...
    implements DataServiceManagerAccess, LiveDataService
{
    ...
    public com.gldb.data.Tbcheckbook getTbcheckbookById(Integer id, PagingOptions pagingOptions) {
        List<com.gldb.data.Tbcheckbook> rtn = ((List<com.gldb.data.Tbcheckbook> ) dsMgr.invoke(taskMgr.getQueryTask(), (GLDBConstants.getTbcheckbookByIdQueryName), id, pagingOptions));
        if (rtn.isEmpty()) {
            return null;
        } else {
            return rtn.get(0);
