...
public class DBServiceImpl implements DBService {
	...
	// IMPORTANT!!!
	private static final String DBNAME = "GLDB";
	/**
	...
	public Object executeUniqueSQLQuery(String sQuery, Map parameters) {
		// ---
		GLDB service = (GLDB) RuntimeAccess.getInstance().getServiceBean(
				DBNAME);
	...
	public int getSQLMaxId(String query, Map parameter) {
		// TODO Auto-generated method stub
		GLDB service = (GLDB) RuntimeAccess.getInstance().getServiceBean(DBNAME);
		int result = 0;
		try{
			service.begin();
			Session session = service.getDataServiceManager().getSession();
	...
	public Object execStoredProc(String query, Map param, Class classname, int rettype, Object obj) {
		// TODO Auto-generated method stub
		/*
		 * ret type 0 = unique result
		 * ret type 1 = list result
		 * ret type 2 = flag result/confirmation/delete
		 * ret type 3 = save
		 * ret type 4 = batch saving
		 * */
		GLDB service = (GLDB) RuntimeAccess.getInstance().getServiceBean(DBNAME);
		try {
			service.begin();
	...
	/**
	 * @param rettype = (0 for unique result, 1 for list result)
	 * @param classname = return form, The result set column names should be same as the column names specified in the form/class.
	 * */
	@SuppressWarnings("rawtypes")
	@Override
	public Object execSQLQueryTransformer(String query, Map params, Class classname, Integer rettype) {
		GLDB service = (GLDB) RuntimeAccess.getInstance().getServiceBean(DBNAME);
		try {
			service.begin();
			Session session = service.getDataServiceManager().getSession();
			...
			logger.warn("ERROR AT DBService.execSQLQueryTransformer: ", ex);
			logger.fatal("Problem executing query " + query + " with parameters"
					+ params, ex);
			service.rollback();
		}
		return null;
